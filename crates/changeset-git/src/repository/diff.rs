...
mod tests {
    ...
    #[test]
    fn uncommitted_changes_detects_unstaged_modification() -> anyhow::Result<()> {
        let (dir, repo) = setup_test_repo()?;
    ...
    #[test]
    fn uncommitted_changes_detects_staged_addition() -> anyhow::Result<()> {
        let (dir, repo) = setup_test_repo()?;
    ...
    #[test]
    fn uncommitted_changes_detects_untracked_file() -> anyhow::Result<()> {
        let (dir, repo) = setup_test_repo()?;
    ...
    #[test]
    fn uncommitted_changes_on_clean_tree_returns_empty() -> anyhow::Result<()> {
        let (_dir, repo) = setup_test_repo()?;
    ...
    #[test]
    fn uncommitted_changes_combines_staged_and_unstaged() -> anyhow::Result<()> {
        let (dir, repo) = setup_test_repo()?;

        fs::write(dir.path().join("committed.txt"), "initial")?;
        let mut index = repo.inner.index()?;
        index.add_path(std::path::Path::new("committed.txt"))?;
        index.write()?;
        ...
        assert_eq!(statuses["committed.txt"], FileStatus::Modified);
